...
export function checkExample(dir) {
  ...
  // `seen` covers the whole tree, for the location and duplicate checks. `seenInSrc` covers only
  // regions under src/, because that is the only place the manifest cross-checks (region-missing,
  // region-undeclared) are defined to look.
  const seen = new Map();
  const seenInSrc = new Map();
  for (const path of sourceFiles(dir)) {
    ...
    const inSrc = relativePath.startsWith("src/");
    for (const region of parsed) {
      ...
      if (inSrc && !seenInSrc.has(region.id)) {
        seenInSrc.set(region.id, relativePath);
      }
    }
  }
  ...
  if (manifest !== null) {
    ...
    for (const [id, file] of seenInSrc) {
      if (!declared.has(id)) {
