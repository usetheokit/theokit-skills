...
/**
 * A changesets snapshot version, which is a throwaway and never a pin.
 *
 * `theokit-examples/README.md` opens with the promise this rule serves: *"Every example installs
 * theokit from npm, at a pinned version, exactly as a stranger would."* A stranger does not install
 * a snapshot — it exists to verify a fix from the registry before a release cut, under a dist-tag
 * that is deliberately not `latest`.
 *
 * The hole was harmless until `usetheokit/theokit-sdk#484` added that path. The workflow is
 * publish, verify, then pin the real release, and nothing made the last step happen; npm versions
 * are immutable after 72 hours, so a snapshot left behind resolves forever while the example
 * demonstrates a tree that was never released.
 *
 * ## What this deliberately does not refuse
 *
 * Every OTHER prerelease. `EXAMPLE-CONTRACT.md` lists `4.61.0-beta.1` as valid, and a published
 * beta is something a stranger can install — an example pinned to one during a migration is making
 * a real claim about a real artefact. Whether the README's promise should exclude those too is a
 * separate decision about the contract, not a hole in the checker, and taking it here would refuse
 * legitimate pins in the same commit that closes an unrelated gap.
 *
 * `changeset version --snapshot <tag>` appends `-<tag>-<YYYYMMDDHHmmss>`, and the tag comes from a
 * dispatch input — so the match is anchored on the trailing 14-digit UTC timestamp, which no
 * hand-authored prerelease identifier carries, rather than on any particular tag.
 */
const SNAPSHOT_VERSION = /-\d{14}$/;
  ...
  if (existsSync(join(dir, "package.json"))) {
    ...
    if (pkg !== null) {
      ...
      for (const [name, range] of Object.entries(allDependencies)) {
        ...
        } else if (SNAPSHOT_VERSION.test(range)) {
          add(
            "exact-pin",
            `${name} is pinned to the snapshot "${range}" — snapshots are published to verify a fix ` +
              "before a release cut, never to be committed. Pin the released version instead.",
          );
        }
      }
    }
