...
export function drift(projectRoot, { version }) {
  ...
  // Content drift, split by HOW the skill got there. Both are real drift and both are reported; what
  // differs is the cause, and therefore the remedy.
  //
  // In `link` mode the installed path is a symlink into `node_modules` and `digestOf` follows it, so
  // the recorded digest describes the DEPENDENCY's bytes. Following the link is deliberate — it is
  // why both install modes needed no special case, and dropping the digest for links would remove
  // the detection this module exists for, on the mode that is the DEFAULT for a stable source. What
  // was wrong was the sentence: a same-version change under the link (a workspace dependency, a
  // `file:` install, a repack) reported "no longer match this version", which is false twice over —
  // the version DOES match, and the user changed nothing. Reinstalling fixes a copy-mode edit; only
  // the dependency moving explains a link-mode one. (B-023, W-07 of the B-002 review.)
  const drifted = manifest.entries.filter((e) => e.digest !== digestOf(entryPath(projectRoot, e)));
  const linked = drifted.filter((e) => e.mode === "link").map((e) => e.path);
  const changed = drifted.filter((e) => e.mode !== "link").map((e) => e.path);
  return drifted.length > 0 ? { kind: "content", changed, linked } : { kind: "current" };
}
